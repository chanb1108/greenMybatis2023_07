...
public class TranController {
	...
	// 에러 발생 시 rollback 필요없음.
	@RequestMapping("/tran/insert1")
	String insert1() {
		try {
		tm.insert1(arrs(70,74,71));
		} catch (Exception e) {
			System.out.println("insert1 에러 발생");
		}
		return "redirect:list";
	}
	
	// 에러 발생 시 rollback 하지 않음 ---Transaction 필요
	@RequestMapping("/tran/insert2")
	String insert2() {
		try {
			tm.insert2(arrs(84,81,85));
		} catch (Exception e) {
			System.out.println("insert2 에러 발생");
		}
		return "redirect:list";
	}
	
	// 에러 발생 시 rollback 하지 않음 ---Transaction 실행 1
	...
	String insert3() {
		...
		try {
			...
			manager.commit(status);		// 정상싱행 시 commit
		} catch (Exception e) {
			System.out.println("insert3 에러 발생");
			manager.rollback(status);	// 에러 시 rollback
		}
		return "redirect:list";
	}
	
	// 에러 발생 시 rollback 하지 않음 ---Transaction 실행2
	@RequestMapping("/tran/insert4")
	String insert4() {
		try {
			service.tranGo();
		} catch (Exception e) {
			System.out.println("insert4 에러 발생");
		}
